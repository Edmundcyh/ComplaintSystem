...
@router.post("/complaints/", dependencies=[Depends(oauth2_scheme), Depends(is_complainer)],
             response_model=ComplaintOut)  # response model is what data to feedback to user
async def create_complaint(request: Request, complaint: ComplaintIn):  # Complaint In schema
    user = request.state.user
